...
class DomainAdversarialABSATrainer:
    ...
    def train_step(self, batch: Dict[str, torch.Tensor], dataset_name: str) -> Dict[str, float]:
        ...
        # Update model's alpha if it has domain adversarial training
        if hasattr(self.model, 'domain_trainer') and self.model.domain_trainer:
            if hasattr(self.model.domain_trainer, 'set_alpha'):
                self.model.domain_trainer.set_alpha(current_alpha)
        ...
        try:
            ...
            # Add domain adversarial components if enabled
            if self.use_domain_adversarial:
                # Domain classification loss
                if 'domain_logits' in outputs:
                    domain_logits = outputs['domain_logits']
                    domain_loss = nn.CrossEntropyLoss()(domain_logits, domain_ids)
                    total_loss = total_loss + self.domain_loss_weight * domain_loss
                    loss_dict['domain_loss'] = domain_loss.item()
                
                # Orthogonal constraint loss
                if 'feature_representations' in outputs:
                    features = outputs['feature_representations']
                    orth_loss = self._compute_orthogonal_loss(features)
                    total_loss = total_loss + self.orthogonal_loss_weight * orth_loss
                    loss_dict['orthogonal_loss'] = orth_loss.item()
        ...
        if self.use_domain_adversarial:
            ...
            if 'domain_loss' in loss_dict:
                self.domain_loss_history.append(loss_dict['domain_loss'])
            if 'orthogonal_loss' in loss_dict:
                self.orthogonal_loss_history.append(loss_dict['orthogonal_loss'])
    ...
    def _compute_orthogonal_loss(self, features: torch.Tensor) -> torch.Tensor:
        ...
        # Normalize features
        features_norm = nn.functional.normalize(features_flat, p=2, dim=1)
        
        # Compute gram matrix
        gram = torch.mm(features_norm.t(), features_norm)
        
        # Orthogonal loss: ||G - I||^2
        identity = torch.eye(hidden_size, device=features.device)
        orth_loss = torch.norm(gram - identity, p='fro') ** 2
